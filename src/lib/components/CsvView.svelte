<script lang="ts">
  ...
  let input: string = "assays/tmp/dataset/df.csv#row=2";
  ...
  onMount(async () => {
    ...
    let csvRow = parts.pop();
    let url = parts.at(0);

    if (!csvRow || !url) {
      console.error("missing url or row");
      return;
    }
    ...
    let df = await DataFrame.fromCSV(new File([result], "file.csv"));
...
<section class="flex flex-col overflow-hidden h-[calc(100vh-150px)]">
  <div class="flex flex-col gap-1 pb-4">
    <div class="collapse bg-base-100 border-base-300 border">
  <input type="checkbox" />
  <div class="collapse-title font-semibold flex items-center "><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-6">
