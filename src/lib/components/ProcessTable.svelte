<script lang="ts">
  ...
  // let processData: any = $state(data);

  $inspect(data, "data for table");

  // TODO: Data updates but table is not rendering new data???
  ...
  // let processes = $derived(data?.about ? data.about.map((e) => arcData.getObjectById(e["@id"])) : []);
  ...
  <div class="dropdown">
    ...
    >
      <div class="flex gap-2">
        <input
          bind:checked={userSettings.options.autoDownloadImages}
          type="checkbox"
          class="checkbox"
        />
  ...
  <Pagination data={tableObjects}>
    {#snippet content(paginatedEntries, currentPage, maxEntries)}
      <div class="bg-base-200/30 grid grid-cols-[min-content_1fr_1fr_1fr]">
        ...
        <div class="p-3 bg-base-200 font-semibold border-b border-secondary">
          Source
        </div>
        <div class="p-3 bg-base-200 font-semibold border-b border-secondary">
          Protocol
        ...
        <div
          class="flex justify-center p-3 bg-base-200 font-semibold border-b border-secondary"
        >
          Sample
        </div>
        ...
        {#each paginatedEntries as process, i (currentPage * maxEntries + i)}
          <!-- Column 1 -->
          <div class="flex justify-center min-h-16 items-center">
            {i + 1 + currentPage * maxEntries}
          </div>
